...
import { commentOnBlogAction } from "@/lib/actions/comments";
import { BlogCommentSchema } from "@/lib/schema/comments";
import {
  Form,
  FormControl,
  FormField,
...
export const CommentForm = ({ blogId }: { blogId: string }) => {
  ...
  function onSubmit(values: z.infer<typeof BlogCommentSchema>) {
    startTransition(() => {
      commentOnBlogAction(values)
        .then((res) => {
          if (res.error) {
